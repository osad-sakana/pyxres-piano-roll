...
- **ピアノロール**: クリックで配置/削除、ドラッグで移動。`←→`で列移動、`↑↓`でノートを半音移調
  （範囲選択中は選択範囲に重なる全ノートが対象。範囲外へはみ出した部分も一緒に動きます。
  範囲に重なる音が1つでも音域端を超える場合は何も動きません）、`Enter`で音符⇔休符
- **選択・コピー・ペースト**: `Shift+←→`／`Shift+クリック`／`Shift+ドラッグ`で範囲選択、
  小節番号ルーラーのクリック／ドラッグで小節単位の範囲選択、`Ctrl(Cmd)+A`で全選択、`Esc`で解除。
  `Ctrl(Cmd)+C`/`X`/`V`でコピー・カット・ペースト（アプリ内のみ、OSクリップボードとは連携しません）。
  貼り付け先の既存ノートは上書きされ、パターン末尾を超える分は切り詰められます。
  選択範囲の前後にはみ出すノートは、範囲外に残る部分を保ったまま範囲内だけが削除されます
  （ノートが2つに分かれることがありますが、選択していない列の音は失われません）。
  そのためカット→同じ位置へ貼り直せば鳴る音は元と変わりません。
  キャレット未選択の状態で貼り付けるとパターン先頭に貼り付けられます。
  tone/volume/effectはコピー対象外です（ノート移動と同じ扱いで、値は列位置に紐づいたまま動きません）
- **音価**: ノート右端をドラッグで長さを変更（Cubase風）。pyxresに音価の概念はないため、書き出し時に同音程の連続ノートへ分割されます
- **曲構造**: グリッドのセルへパターンをドラッグ＆ドロップ（またはクリック）して配置します（4チャンネルまで）。空白セル（1小節の休符）を挟めるため、トラックの途中からでも開始できます。ブロックの✕は空白化（タイミング維持）、空白セルの✕で詰められます
- **BPM**: 曲構造ヘッダで曲ごとに設定（20〜900）
- **移調**: 曲構造ヘッダで曲ごとに±59半音。再生・書き出し時に全ノートへ非破壊で適用され、音域外は端へクランプ（書き出し時に警告表示）
...
内部フォーマットv6。曲（Song）がパターンを内包します。

```
Project
 └─ songs: Song[]                  … 曲数は無制限（書き出し時に8スロットへ選択割り当て）
     ├─ bpm                        … 20〜900。書き出し時 speed = round(1800 / bpm)（1列=16分音符）
     ├─ transpose                  … ±59半音。再生・書き出し時に全ノートへ非破壊で適用
     ├─ timeSignature              … "4/4" | "3/4"。曲全体で1つ。1小節の列数を決める（4/4=16列・3/4=12列）
     ├─ patterns: Pattern[]        … 1曲あたり最大64個
     │   ├─ rateMode               … normal / double(2倍) / half(1/2倍)。speedを1/2倍・2倍に変換
     │   └─ lengths                … 音価（ノートが占有する列数）。書き出し時に同音程の連続ノートへ分割
     └─ channels: (patternId|null)[][] … 最大4チャンネルのグリッド。null=1小節（timeSignature基準の列数）の休符セル。
                                      書き出し時は曲ごとに1つの休符サウンドへ変換（64枠を1つ消費）
```

ピアノロール上部には、パターン内ローカルの小節番号（1始まり）をルーラー表示します。
パターンは曲内で複数回・任意位置に配置されうるため、曲全体を通した小節番号はエディタの外に出ません。

拍子を変えてもパターンの列数（長さ）自体は変わらず、空白セル（休符）の長さのみ追従します。
そのため4/4で組んだ曲を後から3/4へ切り替えると、既存パターンの小節境界がずれて見えることがあります
（データは非破壊で、拍子を戻せば表示も戻ります）。3/4を使う場合は曲の最初から3/4で組むのがおすすめです。
