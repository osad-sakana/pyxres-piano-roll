...
- **選択・コピー・ペースト**: `Shift+←→`／`Shift+クリック`／`Shift+ドラッグ`で範囲選択、`Ctrl(Cmd)+A`で全選択、`Esc`で解除。
  ...
  選択範囲より後ろまで続いていたノートは音価ごと削除されるため、範囲より後ろの列が変化することがあります（クリック削除と同じ挙動）。
  一方、選択範囲より前から続いていたノートは範囲内に残る部分がコピーされるため、
  カット→同じ位置へ貼り直せば（ノートが2つに分かれることはあっても）鳴る音は元と変わりません。
...
Project
 └─ songs: Song[]                  … 曲数は無制限（書き出し時に8スロットへ選択割り当て）
     ...
     └─ channels: (patternId|null)[][] … 最大4チャンネルのグリッド。null=1小節の休符セル。
